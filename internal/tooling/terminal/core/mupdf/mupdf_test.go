...
func TestInstall(t *testing.T) {
	...
	if mc.InstalledPkg != "mupdf" {
		t.Fatalf("expected InstallPackage(%s), got %q", "mupdf", mc.InstalledPkg)
	}
}

// SKIP: ForceInstall test as per guidelines
// ForceInstall calls Uninstall() first which returns error since mupdf uninstall is not supported
...
func TestSoftInstall(t *testing.T) {
	...
	if mc.MaybeInstalled != "mupdf" {
		t.Fatalf("expected MaybeInstallPackage(%s), got %q", "mupdf", mc.MaybeInstalled)
	}
}
...
// 	app := &Mupdf{Cmd: mc}
//
// 	// mupdf doesn't require separate configuration files
...
func TestExecuteCommand(t *testing.T) {
	...
	app := &Mupdf{Cmd: mc, Base: mockBase}

	// Test 1: Successful execution
	t.Run("successful execution", func(t *testing.T) {
		mockBase.SetExecCommandResult("mutool 1.23.0", "", nil)
		...
		if lastCall.Command != "mupdf" {
			t.Fatalf("Expected command 'mupdf', got %q", lastCall.Command)
		}
	...
	t.Run("command execution error", func(t *testing.T) {
		...
		mockBase.SetExecCommandResult(
			"",
			"command not found",
			fmt.Errorf("command not found: mupdf"),
		)
		...
		if !strings.Contains(err.Error(), "failed to run mupdf command") {
			t.Fatalf("Expected error to contain 'failed to run mupdf command', got: %v", err)
		}

		// Verify the error was properly wrapped
		if !strings.Contains(err.Error(), "command not found: mupdf") {
	...
	t.Run("multiple arguments", func(t *testing.T) {
		...
		err := app.ExecuteCommand("draw", "-o", "output.png", "input.pdf")
		if err != nil {
			t.Fatalf("ExecuteCommand failed: %v", err)
		}

		lastCall := mockBase.GetLastExecCommandCall()
		expectedArgs := []string{"draw", "-o", "output.png", "input.pdf"}
	...
	t.Run("no arguments", func(t *testing.T) {
		mockBase.ResetExecCommand()
		mockBase.SetExecCommandResult("mupdf info", "", nil)
