...
func TestInstall(t *testing.T) {
	...
	if mc.InstalledPkg != "gdbm" {
		t.Fatalf("expected InstallPackage(%s), got %q", "gdbm", mc.InstalledPkg)
	}
}

// SKIP: ForceInstall test as per guidelines
// ForceInstall calls Uninstall() first which returns error since gdbm uninstall is not supported
...
func TestSoftInstall(t *testing.T) {
	...
	if mc.MaybeInstalled != "gdbm" {
		t.Fatalf("expected MaybeInstallPackage(%s), got %q", "gdbm", mc.MaybeInstalled)
	}
}
...
// 	app := &Gdbm{Cmd: mc}
//
// 	// gdbm doesn't require separate configuration files
...
func TestExecuteCommand(t *testing.T) {
	...
	app := &Gdbm{Cmd: mc, Base: mockBase}

	// Test 1: Successful execution
	t.Run("successful execution", func(t *testing.T) {
		mockBase.SetExecCommandResult("gdbm 1.23", "", nil)
		...
		if lastCall.Command != "gdbm" {
			t.Fatalf("Expected command 'gdbm', got %q", lastCall.Command)
		}
	...
	t.Run("command execution error", func(t *testing.T) {
		...
		mockBase.SetExecCommandResult(
			"",
			"command not found",
			fmt.Errorf("command not found: gdbm"),
		)
		...
		if !strings.Contains(err.Error(), "failed to run gdbm command") {
			t.Fatalf("Expected error to contain 'failed to run gdbm command', got: %v", err)
		}

		// Verify the error was properly wrapped
		if !strings.Contains(err.Error(), "command not found: gdbm") {
	...
	t.Run("multiple arguments", func(t *testing.T) {
		...
		err := app.ExecuteCommand("--check", "database.db")
		if err != nil {
			t.Fatalf("ExecuteCommand failed: %v", err)
		}

		lastCall := mockBase.GetLastExecCommandCall()
		expectedArgs := []string{"--check", "database.db"}
	...
	t.Run("no arguments", func(t *testing.T) {
		mockBase.ResetExecCommand()
		mockBase.SetExecCommandResult("gdbm info", "", nil)
